...
# Tutorial Monitor Validator Cosmos Network


### Install exporters di validator node kalian
Istall otomatis dengan command dibawah
```
...
| KEY |VALUE |
|---------------|-------------|
| **bond_denom** | Nama token dalam denominasi, misalnya, `ubld` untuk Agoric. untuk filenya ada di genesis |
| **bench_prefix** | Address chain awalan, Contoh disini, `agoric` untuk chain Agoric. Anda dapat menemukannya di alamat publik seperti ini **agoric**_valoper1zyyz4m9ytdf60fn9yaafx7uy7h463n7alv2ete_ |
| **rpc_port** | Validator dan port `rpc` yang didefinisikan dalam file `config.toml`. Nilai default untuk aura adalah `26657` |
| **grpc_port** | Port `grpc` validator Anda yang didefinisikan dalam file `app.toml`. Nilai default untuk aura adalah `9090` |
pastikan prometheus diaktifkan di file validator `config.toml`

mPastikan berikut terbuka:
- `9100` (node-exporter)
- `9300` (cosmos-exporter)
- `26660` (validator prometheus)

## Deployment
Monitoring Network tidak perlu diterapkan pada VPS yg berbeda ataupun VPS yang spek tinggi VPS yang termurah support untuk dapat memberi tahu jika validator mati/hidup!
...
```
wget -O install_monitoring.sh https://raw.githubusercontent.com/Art-Sy5team/Art-Sy5team-Tutorial/main/cosmos_node_monitoring/install_monitoring.sh && chmod +x install_monitoring.sh && ./install_monitoring.sh
```
...
| KEY | VALUE |
|---------------|-------------|
| TELEGRAM_ADMIN | USER ID telegtam anda bisa check [@userinfobot](https://t.me/userinfobot). Silhakan start untuk mendapatkan ID anda |
